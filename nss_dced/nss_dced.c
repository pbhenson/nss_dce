...
pthread_addr_t handle_request(pthread_addr_t arg)
{
  ...
  TRACE("nss_dced.handle_request(%d): initializing registry handle\n", sock);
  sec_rgy_site_open_query(NULL, &rgy_context, &dce_status);

  if (dce_status == sec_login_s_no_current_context)
    {
      TRACE("nss_dced.handle_request(%d): handle init failed, trying with no auth\n", sock);
      auth_info.info_type = sec_rgy_bind_auth_none;
      sec_rgy_site_bind_query(NULL, &auth_info, &rgy_context,
                              &dce_status);
    }

  TRACE("nss_dced.handle_request(%d): entering request loop\n", sock);
  while (read(sock, &request, sizeof(int)))
    {
      switch (request)
        {
        case NSS_DCED_GETPWNAM:
          TRACE("nss_dced.handle_request(%d): received NSS_DCED_GETPWNAM\n", sock);
          read(sock, &string_length, sizeof(int));
          read(sock, pname, string_length);
          sec_rgy_cursor_reset(&account_cursor);
          TRACE("nss_dced.handle_request(%d): lookup for username %s\n", sock, pname);
          nss_dced_acct_lookup(rgy_context, &account_cursor, sock, pname);
          break;

        case NSS_DCED_GETPWUID:
          TRACE("nss_dced.handle_request(%d): received NSS_DCED_GETPWUID\n", sock);
          read(sock, &uid, sizeof(uid_t));
          sec_rgy_cursor_reset(&account_cursor);
          TRACE("nss_dced.handle_request(%d): lookup for UID %d\n", sock, uid);
          nss_dced_getpwuid(rgy_context, &account_cursor, sock, uid);
          break;
        ...
        case NSS_DCED_GETPWENT:
          ...
          nss_dced_acct_lookup(rgy_context, &account_cursor, sock, "");
          break;
        ...
        case NSS_DCED_GETGRENT:
          ...
          nss_dced_getgrent(rgy_context, &group_cursor, sock);
          break;
  ...
  sec_rgy_site_close(rgy_context, &dce_status);
  close(sock);
}

void nss_dced_acct_lookup(sec_rgy_handle_t rgy_context, sec_rgy_cursor_t *account_cursor,
                          int sock, sec_rgy_name_t pname)
{
  ...
  strcpy(name_key.gname, "");
  strcpy(name_key.oname, "");

  sec_rgy_acct_lookup(rgy_context, &name_key, account_cursor,
  ...
  write(sock, &string_length, sizeof(int));
  ...
  uid = unix_sid.person;
  write(sock, &uid, sizeof(uid_t));

  gid = unix_sid.group;
  write(sock, &gid, sizeof(gid_t));
...
void nss_dced_getpwuid(sec_rgy_handle_t rgy_context, sec_rgy_cursor_t *account_cursor,
                       int sock, uid_t uid)
{
  ...
  sec_rgy_pgo_unix_num_to_name(rgy_context, sec_rgy_domain_person,
  ...
  nss_dced_acct_lookup(rgy_context, account_cursor, sock, pgo_name);
}

void nss_dced_getgrnam(sec_rgy_handle_t rgy_context, int sock, sec_rgy_name_t pname)
{
  error_status_t dce_status;
  ...
  sec_rgy_cursor_reset(&group_cursor);
  sec_rgy_pgo_get_by_name(rgy_context, sec_rgy_domain_group, pname, &group_cursor,
                          &pgo_item, &dce_status);
...
void nss_dced_getgrgid(sec_rgy_handle_t rgy_context, int sock, gid_t gid)
{
  sec_rgy_name_t pgo_name;
  error_status_t dce_status;
  int response;
  ...
  sec_rgy_pgo_unix_num_to_name(rgy_context, sec_rgy_domain_group, gid, 
                               pgo_name, &dce_status);
  ...
  TRACE("nss_dced.getgrgid(%d): calling getgrnam for groupname %s\n", sock, pgo_name);
  nss_dced_getgrnam(rgy_context, sock, pgo_name);
}

void nss_dced_getgrent(sec_rgy_handle_t rgy_context, sec_rgy_cursor_t *group_cursor, int sock)
{
  sec_rgy_name_t pgo_name;
  error_status_t dce_status;
  sec_rgy_pgo_item_t pgo_item;
  ...
  sec_rgy_pgo_get_next(rgy_context, sec_rgy_domain_group, "", group_cursor, &pgo_item,
                       pgo_name, &dce_status);
